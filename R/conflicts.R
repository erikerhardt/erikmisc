# Based on conflicts.R from the tidyverse package, https://github.com/tidyverse/tidyverse/blob/main/R/conflicts.R

#' Conflicts between the erikmisc and other packages
#'
#' This function lists all the conflicts between packages in the erikmisc
...
erikmisc_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", erikmisc_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "erikmisc_conflicts")
}

erikmisc_conflict_message <- function(x) {
...
#' @export
print.erikmisc_conflicts <- function(x, ..., startup = FALSE) {
