...
#' @param var_plot variables to plot against \code{dx_current}
...
#'
#' @examples
f_compare_plot <- function(dat_sub, var_plot, var_name = NULL, var_subname = NULL, comparisons = c(NULL, "all", list())) {
  ...
  dat_analy <- subset(dat_plot, is.finite(var) & !is.na(dx_current))
  cm_result <- ggpubr::compare_means(var ~ dx_current, data = dat_analy)
  print(cm_result)

  if (comparisons == "all") {
    # better ordering for plot of all comparisons
    list_group <- as.character(sort(unique(dat_sub$dx_current)))
    my_comparisons <- list()
    i_list <- 0
  ...
  p.pubr2 <- ggpubr::add_summary(p.pubr2, fun = "mean_ci", size = 0.75, width = 0.5, color = "red", error.plot = "errorbar")
  ...
  p.pubr2 <- p.pubr2 + ggpubr::stat_compare_means(label.y = min(dat_analy$var, na.rm = TRUE) - 0.2 * diff(range(dat_analy$var, na.rm = TRUE))
                                                , label.x = min(as.numeric(dat_sub$dx_current)) + diff(range(unique(as.numeric(dat_sub$dx_current)))) * 0.2
                                                , size = 3  # text size in plot
                                                )     # Add global p-value
  p.pubr2 <- ggpubr::ggpar(p.pubr2
                , main = var_name
                , submain = var_subname # "by Diagnosis with mean and 95% CI"
                , xlab = "Diagnosis"
                , ylab = var_plot
                )
  #print(p.pubr2)
